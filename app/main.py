...
# --- НОВЫЙ ЭНДПОИНТ: Получение списка чатов пользователя ---
@app.get("/users/{user_id}/chats", response_model=List[schemas.ChatInfo])
    ...
    async def load_and_send_history(chat_id: int):
        ...
        try:
            ...
            for msg in messages_from_db:
                ...
                role = "user" if pydantic_msg.sender == schemas.SenderType.USER else "assistant"
    ...
    try:
        ...
        while True:
            ...
            user_message_schema = schemas.MessageCreate(
                text=user_text,
                sender=schemas.SenderType.USER # Используем Enum напрямую
            )
            try:
            ...
            bot_message_schema = schemas.MessageCreate(
                text=bot_response_text,
                sender=schemas.SenderType.BOT # Используем Enum напрямую
            )
            try:
